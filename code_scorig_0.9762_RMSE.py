...
if __name__ == "__main__":
	...
	user_features = sc.textFile(folder_path+'user.json', 8).map(lambda row: json.loads(row)).filter(lambda row: row['user_id'] in distinct_user).map(lambda row: (user_to_encoding[row['user_id']], np.array([row['average_stars'], row['review_count'], int(row['yelping_since'][2:4]), row['useful'], row['funny'], row['cool'], row['fans'], len(row['friends']), len(row['elite'])], dtype=np.float32))).collectAsMap()
	...
	tips_features = sc.textFile(folder_path+'tip.json', 8) \
		...
		.filter(lambda row: row['user_id'] in distinct_user) \
		.filter(lambda row: row['business_id'] in distinct_bizz) \
		.map(lambda row: ((user_to_encoding[row['user_id']], bizz_to_encoding[row['business_id']]), (np.array(list(VADER_sentiment_analyzer.polarity_scores(row['text']).values()), dtype=np.float32), 
